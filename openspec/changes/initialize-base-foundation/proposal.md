...
- Add the initial Nuxt Layer structure, strict TypeScript, Tailwind CSS v4, shadcn-vue-compatible semantic tokens, Reka UI dependencies, and the shared `cn()` utility.
- Use shadcn-vue as source-distributed `Ui*` primitives and define public `Block*` composition conventions for generic marketing and admin UI patterns, excluding business-specific behavior.
...
- Adds shadcn-vue configuration and source-owned UI primitives; no complete UI component suite or business features are added in Sprint 1.
