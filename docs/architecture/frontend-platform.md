...
| --- | --- | --- | --- |
| landify-base | Nuxt Layer | UI, tokens, composables, Nuxt config, Storybook | Nuxt `extends` remote Git layer |
...
- Storybook để preview, documentation và accessibility checks; Storybook không đi vào production dependency graph.
...
export default defineNuxtConfig({
  extends: [
    'github:yourname/landify-base#v1.4.0',
  ],
})
```

Khi `landify-base` phát hành v1.5.0, Page A/B vẫn ở v1.4.0 cho tới khi chủ động đổi tag và kiểm thử. Đây là khác biệt cốt lõi với template.

## 3. landify-tooling

`landify-tooling` là package riêng vì ESLint/Prettier chạy độc lập với Nuxt Layer. Tách riêng không làm Nuxt Layer mất khả năng sử dụng; ngược lại chính `landify-base` cũng nên dùng chung tooling này.
...
- Demo chỉ đủ để kiểm tra Nuxt, base components, CSS/theme và wiring hoạt động.
...
| landify-tooling | Theo package/lockfile | Nâng package version và chạy install. |
...
- ESLint/Prettier dùng shareable package thay vì copy rule vào từng repository.
